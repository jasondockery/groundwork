...
arch="$(uname -m)"
case "$arch" in
  x86_64) rust_arch="x86_64"; go_arch="amd64"; deb_arch="amd64" ;;
  aarch64|arm64) rust_arch="aarch64"; go_arch="arm64"; deb_arch="arm64" ;;
...
install_lazygit() {
  tmp="$(mktemp -d)"
  url="$(latest_asset jesseduffield/lazygit "Linux_${rust_arch}.*\\.tar\\.gz$")"
...
install_delta() {
  tmp="$(mktemp -d)"
  url="$(latest_asset dandavison/delta "_${deb_arch}\\.deb$")"
...
install_sesh() {
  tmp="$(mktemp -d)"
  url="$(latest_asset joshmedeski/sesh "Linux_${go_arch}.*\\.tar\\.gz$")"
