...
- Support contract: Groundwork is fully supported on macOS, supports its Unix development core on native Linux, and supports Windows through WSL2. Native Windows shells (PowerShell, CMD, Git Bash) are not a v1 target and must fail closed with WSL2 guidance rather than partially work. Platform-specific behavior is explicit (`groundwork-platform` reports darwin/linux/wsl/unsupported), and Groundwork never mutates a host operating system outside the environment it runs in: no distro package upgrades on Linux, no Windows-host updates from WSL2.
