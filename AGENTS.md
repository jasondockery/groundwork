...
- Support contract: Groundwork fully supports macOS and the Unix developer core on Ubuntu LTS and Debian stable, natively or under WSL2; Ubuntu LTS is the primary tested Windows path, and other mainstream distributions are supported as their coverage lands. Groundwork supports Linux directly — WSL2 is one supported way to run that Linux environment on a Windows-owned machine, and native Linux is the recommendation for anyone who wants no Microsoft dependency. WSL1 and native Windows shells (PowerShell, CMD, Git Bash) are unsupported and must fail closed with guidance rather than partially work. Platform and distribution are separate dimensions (`groundwork-platform` reports darwin/linux/wsl2/wsl1/unsupported; `groundwork-distro` reports the distribution); only bootstrap prerequisites may branch on distribution — shared Unix behavior stays shared. Groundwork never mutates a host operating system outside the environment it runs in: no distro package upgrades on Linux, no Windows-host updates from WSL2.
