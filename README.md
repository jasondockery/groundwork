...
- [ ] **Manual ChatGPT ownership** — `update-all` reports a ChatGPT app that exists outside Homebrew and continues every safe lane without taking it over. If you own the app and want Homebrew updates, quit it and run `groundwork-app-adopt chatgpt`; device-managed Macs remain administrator-owned.
- [ ] **ChatGPT app cleanup** — fresh Groundwork setup installs only the current unified `ChatGPT.app`. `ChatGPT Classic.app` remains supported for compatibility and existing Enterprise workflows, but it is a legacy duplicate for most people. Keep it knowingly, or run `groundwork-app-remove chatgpt-classic` for a locked, signing-team-checked, consent-gated move to Trash or exact cask removal. A deprecated `codex-app` receipt stays separate and uses `groundwork-app-remove codex-app`; neither cleanup runs from setup or `update-all`.
