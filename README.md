...
Run `update-all` as the one normal machine-maintenance command. Under one machine-wide lock, it synchronizes Groundwork, upgrades only the Homebrew formulae and ordinary checksum-safe casks declared by Groundwork, refreshes mise-managed tools such as Node LTS and Homebrew-managed AI tools such as Codex and OpenCode, then automatically runs supported package-manager maintenance and the label-scoped Docker scratch tidy. Before a cask mutation, it verifies one exact metadata snapshot. Self-updating casks and replacements that remove launchd services or privileged helper files stay owner-run and appear as Review items with exact commands; Docker Desktop is currently in that lane. Chrome's official stable/Beta casks are the explicit no-checksum exception and update through Google's updater; unmanaged Homebrew software is never pulled into the upgrade scope. pnpm automation accepts only exact pins from canonically discovered Git repositories and standard or explicitly trusted store roots; conflicts and custom stores remain review-only. Docker automation accepts only aged images and exited containers that explicitly opted in through Groundwork's ephemeral labels and scratch-only tag contract. On a desktop Mac, the final post-update health check reports an unusually large Karabiner Core Service footprint or a Karabiner version change. With a terminal it offers the verified restart and defaults to no; without one it prints `groundwork-karabiner-restart` and changes nothing. `chezmoi update`, `init`, and `apply` install newly declared packages but do not request routine upgrades of already-installed ones or restart an already-running input service merely because they applied files; Homebrew may still update a dependency when a new package requires it. Failed managed Homebrew upgrades and cleanup failures preserve completed work, exit nonzero, and keep the shell reminder active until repaired; a recognized half-installed Caskroom collision names the cask and prints owner-run force-remove/reinstall commands without touching application data. Repository dependency installations such as `node_modules`, whole pnpm store generations, unlabeled or legacy tagged Docker images, daemon-wide Docker cache, Xcode data, and application or user data stay owner-confirmed; Homebrew may remove installed orphan formula dependencies while retaining requested formulae and casks. Claude Code stays on its vendor-supported latest channel. Upgraded tools take effect at your next shell prompt, even in already-open terminals; set `GROUNDWORK_UPDATE_REMINDER=0` in `~/.zshrc.local` to silence the stale/pending reminder.
